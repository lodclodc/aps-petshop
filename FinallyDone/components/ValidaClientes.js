...
import {View,Text,TouchableOpacity,FlatList,Alert,Modal,Platform} from "react-native";
  ...
  return (
    <View style={globalStyles.container}>
      ...
      >
        ...
        <Picker.Item label="Cortar unha" value="cortar_unha" />
      </Picker>
...
const excluirAgendamento = async (id) => {
    ...
    const performDeletion = async () => {
        try {
            const dados = await AsyncStorage.getItem("usuarios");
            let listaAtualizada = dados ? JSON.parse(dados) : [];

            if (!Array.isArray(listaAtualizada)) {
                listaAtualizada = [];
            }
            
            const novaLista = listaAtualizada.filter((agendamento) => agendamento.id !== id);
    ...
    if (Platform.OS === "web") {
        ...
        if (confirmDelete) await performDeletion();
    } else {
        Alert.alert(
            "Confirmar Exclusão",
            ...
            [
                ...
                { text: "Excluir", onPress: performDeletion, style: "destructive" },
            ],
            { cancelable: true }
        );
    }
